...
class KMeansClustering:
    ...
    def e_step(self, data):
        ...
        for pt in data:
            ...
            while i<l:
                ...
                distance = self.findDistance(pt,centroid) 
    ...
    def m_step(self, data, cluster_assgn):
        ...
        for i in range(len(cluster_assgn)):
            if cluster_assgn[i] not in dictionary:
                dictionary[cluster_assgn[i]]=[]
            dictionary[cluster_assgn[i]].append(data[i])
        ...
        for key in dictionary:
            ...
            for i in range(len(arr)):
                arr[i] = (arr[i]/len(dictionary[key]))
            
            self.centroids[key] = arr
    ...
    def evaluate(self, data, cluster_assign):
        ...
        for i in range(len(data)):
            pt = data[i]
            distance = self.findDistance(pt,self.centroids[cluster_assign[i]])
            performance += (distance**2)
        return performance
